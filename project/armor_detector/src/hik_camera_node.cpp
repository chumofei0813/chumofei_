    ...
    : Node("hik_camera_node")
    {
        ...
        if (!initCamera()) {
            RCLCPP_FATAL(get_logger(),
                "Camera init failed — node will stay alive but idle. "
                "Check: SDK installed at /opt/MVS? Camera connected via USB?");
            return;
        }
...
private:
    ...
    bool initCamera()
    {
        ...
        if (nRet != MV_OK) {
            RCLCPP_ERROR(get_logger(), "MV_CC_Initialize failed");
            return false;
        }
        ...
        if (nRet != MV_OK || stDeviceList.nDeviceNum == 0) {
            RCLCPP_ERROR(get_logger(), "No Hik camera found");
            return false;
        }

        nRet = MV_CC_CreateHandle(&handle_, stDeviceList.pDeviceInfo[0]);
        if (nRet != MV_OK) {
            RCLCPP_ERROR(get_logger(), "CreateHandle failed");
            return false;
        }

        nRet = MV_CC_OpenDevice(handle_);
        if (nRet != MV_OK) {
            RCLCPP_ERROR(get_logger(), "OpenDevice failed");
            return false;
        }

        nRet = MV_CC_StartGrabbing(handle_);
        if (nRet != MV_OK) {
            RCLCPP_ERROR(get_logger(), "StartGrabbing failed");
            return false;
        }
    ...
    void grabImage()
    {
        ...
        if (nRet != MV_OK) {
            RCLCPP_WARN(this->get_logger(),
                "GetImageBuffer failed: 0x%x", nRet);
            return;
        }
